export async function getPoet(slug) {
		let data;
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/api/poets/${slug}`, {
				method: 'GET',
				headers: {
...
export async function getFeaturedPoets() {
	...
	let data;
		...
		try {
			const response = await fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/api/poets/get-featured-poets`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json'
				},
