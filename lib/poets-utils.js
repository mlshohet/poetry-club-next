export async function getPoet(slug) {
	...
	console.log("In getPoet");
		let data;
		try {
			const response = await fetch(`http://${process.env.VERCEL_URL}/api/poets/${slug}`, {
				method: 'GET',
				headers: {
...
export async function getFeaturedPoets() {
	...
	let data;
		...
		try {
			const response = await fetch(`http://${process.env.VERCEL_URL}/api/poets/get-featured-poets`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json'
				},
